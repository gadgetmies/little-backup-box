...
Contract for `GET /api/display/status` and the `<DisplayStatus>` component: how the latest display frame written by the device-side scripts is surfaced in the webapp.
